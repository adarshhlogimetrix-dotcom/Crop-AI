...
class Land_Preperation extends StatefulWidget {
  const Land_Preperation({super.key});

  @override
  State<Land_Preperation> createState() => land_preperation();
}

class land_preperation extends State<Land_Preperation> {
  // Form Key
...
/*  final TextEditingController _majorMaintenanceController = TextEditingController();*/
 ...
 // String? _selectedSoilCondition = 'Select Soil Condition';
...
/*
  ...
  List<String> _soilConditionName = ['Select Soil Condition'];
  String? _selectedSoilCondition = 'Select Soil Condition';
...
// List to store display category names
  ...
  // This method handles the system back button press
...
setState(() {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // Get soil condition ID
        int soilConditionId = 0;
        if (_selectedSoilCondition != null && _selectedSoilCondition != 'Select Soil Condition') {
          // Find the matching soil condition in the original data
          for (var condition in _soilConditions) {
            if (condition['soil_condition'] == _selectedSoilCondition) {
              soilConditionId = condition['id'];
              break;
            }
          }
        }
        ...
        final Map<String, dynamic> requestData = {
          ...
          'soil_condition_id': soilConditionId,
        ...
        // Make API call
        final response = await http.post(
          Uri.parse('https://ccbfsolution.pmmsapp.com/api/land-preparation-store'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildCustomDropdown(
                    ...
                    selectedValue: _selectedSoilCondition,
                    items: _soilConditionName,
                    onChanged: (value) {
                      if (value != null) {
                        setState(() {
                          _selectedSoilCondition = value;
                        });
                      }
                    },
                  ),
  ...
  void _resetForm() {
    setState(() {
      ...
      _selectedSoilCondition = 'Select Soil Condition';
  ...
  Future _fetchSoilConditions() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String token = prefs.getString('auth_token') ?? '';
      ...
      final response = await http.post(
        Uri.parse('https://ccbfsolution.pmmsapp.com/api/soil-condion'),
        headers: {
          'Authorization': 'Bearer $token',
          'Accept': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success' && responseData['data'] != null) {
          setState(() {
            // Store the full soil condition data
            _soilConditions = List<Map<String, dynamic>>.from(responseData['data']);

            // Extract soil condition names for dropdown
            _soilConditionName = ['Select Soil Condition'];
            _soilConditionName.addAll(_soilConditions.map((item) => item['soil_condition'].toString()).toList());
          });
        }
      } else {
        // Show error message for any non-200 status code
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Something went wrong')),
        );
        print('Failed to load soil conditions: ${response.statusCode}');
      }
    } on SocketException {
      ...
      print('Error fetching soil conditions: $e');
    }
  }
