...
class PostIrrigation extends StatefulWidget {
  const PostIrrigation({super.key});

  @override
  State<PostIrrigation> createState() => postIrrigation();
}

class postIrrigation extends State<PostIrrigation> {
  // Form Key
...
//  String? _selectedManPower = 'Select Man Power';
 ...
 /* DateTime? _startDate;
  ...
  TimeOfDay? _endTime;
  DateTime? _sowingDate; // For Day After Sowing date


  String formattedSowingDate = ''; // For storing the formatted sowing date
*/

  DateTime? _startDate = DateTime.now(); // Initialize with current date
  TimeOfDay? _startTime = TimeOfDay.now(); // Initialize with current time
  TimeOfDay? _endTime = TimeOfDay.fromDateTime(
      DateTime.now().add(const Duration(hours: 4)) // Initialize with current time + 4 hours
  );
  DateTime? _sowingDate = DateTime.now(); // Initialize with current date
...
// Update area based on selected plot
  ...
  Future<void> _selectDate(BuildContext context, bool isStartDate) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: isStartDate ? (_startDate ?? DateTime.now()) : DateTime.now(),
...
// Added missing method for selecting sowing date
  Future<void> _selectSowingDate(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        _sowingDate = picked;
        formattedSowingDate = DateFormat('yyyy-MM-dd').format(_sowingDate!);
      });
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    //      'category': selectedCategories.toList(), // Convert Set to List
       //   'manpower_type': _selectedManPowerRoll == 'Select ManPower' ? null : _selectedManPowerRoll,
          ...
          'day_ofter_swowing': formattedSowingDate, // Use the formatted sowing date
        ...
        // Make API call
        final response = await http.post(
          Uri.parse('https://ccbfsolution.pmmsapp.com/api/post-irrigation'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildCustomTextField(
                    ...
                    keyboardType: const TextInputType.numberWithOptions(decimal: true), // Number keyboard with decimal
                    maxLength: 13, // Limit to 4 digits
                  ),
                  ...
                  // Day After Sowing Date Picker
                  _buildDateTimePicker(
                    labelText: 'Day After Sowing',
                    hintText: 'dd-mm-yyyy',
                    value: _sowingDate == null
                        ? null
                        : DateFormat('dd-MM-yyyy').format(_sowingDate!),
                    icon: Icons.calendar_today,
                    onTap: () => _selectSowingDate(context),
