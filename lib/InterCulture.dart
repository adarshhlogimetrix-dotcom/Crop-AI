...
//  List<String> _manPowerName = ['Select Man Power'];
  ...
  final List<String> _activities = [
    'Select Type', 'Weeding', 'Hoeing', 'Earthing-up', 'Thinning','Gap Filling','Mulching'
  ];
...
/*
  ...
  Future<void> _fetchSiteBlocksPlots() async {
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          final List<dynamic> sitesData = responseData['data'];

          setState(() {
            // Store sites data but don't use it for dropdown
            _sites = List<Map<String, dynamic>>.from(sitesData.map((site) => {
              'site_id': site['site_id'],
              'site_name': site['site_name'],
              'blocks': site['blocks'],
            }));

            // Directly populate the blocks dropdown with all blocks from all sites
            _blocks = [];
            _blockNames = ['Select Block'];

            // Loop through all sites and collect all blocks
            for (var site in _sites) {
              final List<dynamic> blocksData = site['blocks'];
              for (var block in blocksData) {
                _blocks.add({
                  'block_id': block['block_id'],
                  'block_name': block['block_name'],
                  'plots': block['plots'],
                  'site_id': site['site_id'],  // Store parent site ID for reference
                  'site_name': site['site_name'],  // Store parent site name for reference
                });
...
// Update plots based on selected block
  void _updatePlots(String blockName) {
    ...
    if (selectedBlock.isNotEmpty) {
      setState(() {
        _selectedBlockId = selectedBlock['block_id'];
        // Store the selected site ID internally even though it's not shown in UI
        _selectedSiteId = selectedBlock['site_id'];
...
// Update area based on selected plot
  void _updateAreaText(String plotName) {
    ...
    if (selectedPlot.isNotEmpty) {
      setState(() {
        _selectedPlotId = selectedPlot['plot_id'];
        _selectedArea = selectedPlot['area'];
        // Set the area text directly
        _areaText = '${selectedPlot['area']} sqm';
      });
    }
  }
*/
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: showMachineDropdown ? Icon(Icons.search, color: Color(0xFF6B8E23)) : null,
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                ...
                filled: true,
                fillColor: Colors.white,
                hintText: selectedMachines.isEmpty ? 'Search or select machine (Optional) ' : selectedMachines.join(', '),
                hintStyle: TextStyle(
                  color: selectedMachines.isEmpty ? Colors.grey : Colors.black,
                  fontFamily: "Poppins",
                  fontSize: 14,
      ...
      child: AbsorbPointer(
        child: TextFormField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: Icon(icon, color: Color(0xFF6B8E23)),
          ),
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        /*int machineId = 0;
        if (_selectedMachine != null && _selectedMachine != 'Select Machine') {
          machineId = _machineName.indexOf(_selectedMachine!);
          if (machineId == 0) machineId = 0; // Default to 0 for "Select Machine"
          else machineId = machineId; // Actual index for selected machine
        }*/
       ...
       /* List<String> machineIds = [];
        ...
        // Get tractor ID from selection
        /* int tractorId = 0;
        if (_selectedTractor != null && _selectedTractor != 'Select Tractor') {
          tractorId = _tractorName.indexOf(_selectedTractor!);
          if (tractorId == 0) tractorId = 0; // Default to 0 for "Select Tractor"
          else tractorId = tractorId; // Actual index for selected tractor
        }*/
      ...
      /*  List<String> tractorIds = [];
        ...
        // Tractor IDs - यहाँ optional logic है
        List<String> tractorIds = [];
        if (selectedTractors.isNotEmpty) {
          for (String tractorName in selectedTractors) {
            int index = _tractorName.indexOf(tractorName);
     ...
     //     'category': selectedCategories.toList(), // Convert Set to List
        ...
        // Make API call
        final response = await http.post(
          Uri.parse('https://ccbfsolution.pmmsapp.com/api/inter-culture'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildCustomDropdown(
                        labelText: 'Tractor',
                        selectedValue: _selectedTractor,
                        items: _tractorName,
                        onChanged: (value) {
                          if (value != null) {
                            setState(() {
                              _selectedTractor = value;
                            });
                          }
                        },
                      ),
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          setState(() {
            // Create a new list with the default option
            _machineName = ['Select Machine'];

            // Add all machine names, ensuring no duplicates
            _machineName.addAll(machineData.map((machine) => machine.toString()).toList());

            // Make sure the selected machine is valid and set
            _selectedMachine = 'Select Machine';
          });
        } else {
          throw Exception('API returned false success status');
...
/////////////////////////////////////////// In this drop down selecting multiple tractor /////////////////////////////
/*
  Future<void> _fetchTractorDetails() async {
    try {
      // Get token from SharedPreferences
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 /* Future<void> fetchCategories() async {
    ...
    // DEBUG fallback (use this to test without SharedPreferences)
    if (token == null || token.isEmpty) {
      print('⚠️ Token not found in SharedPreferences');
      // token = 'paste_token_here_temporarily'; // UNCOMMENT for testing manually
      return;
    }
