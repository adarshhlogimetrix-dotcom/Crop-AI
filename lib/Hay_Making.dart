...
*/
  ...
  String? _selectedMethodHayMaking = 'Select Method';
...
//  List<String> _manPowerName = ['Select Man Power'];
  ...
  final List<String> _methodHaymakingName = [
    'Select Method', 'Manual', 'Mechanical-Sun Drying', 'Tedding & Raking','Baling','Silage','Windrow','Haylage'
  ];
...
//   print('Plot Name: ${plot['plot_name']}');
//   print('Area: ${plot['area']}');
//   print('Is Fodder Crop: ${plot['is_fodder_crop']}');
//   print('Seed Name: ${plot['seed_name']}');
//   print('Yield MT: ${plot['yield_mt']}');
//   print('----------------------');
// }
...
// New method to update seed names for the entire block
  void _updateSeedNamesForBlock() {
    setState(() {
      ...
      for (var plot in _plots) {
        if (plot['seed_name'] != null && plot['seed_name'].toString().isNotEmpty) {
      ...
      print('Available seed names: ${uniqueSeedNames.toList()}');
    });
  }
  ...
  return TextFormField(
    ...
    decoration: InputDecoration(
      ...
      enabledBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(20.0),
        borderSide: BorderSide(
          color: isFieldEnabled ? Colors.grey.shade400 : Colors.grey.shade300,
          width: 1
        ),
      ),
      focusedBorder: OutlineInputBorder(
          ...
          children: [
            GestureDetector(
              ...
              child: InputDecorator(
                decoration: InputDecoration(
                  labelText: 'Category of Man Power', // Removed asterisk (*) since it's optional
                  errorText: state.errorText,
                  labelStyle: TextStyle(
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          if (_selectedBlockName == 'Select Block' || 
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Please select Block, Plot, and Seed Name first'),
...
Future<void> _submitBasicForm() async {
  ...
  try {
    ...
    final response = await http.post(
      Uri.parse('https://ccbfsolution.pmmsapp.com/api/store-hay-making'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
...
Future<void> _submitForm() async {
  if (_formKey.currentState!.validate()) {
    ...
    try {
      ...
      // Don't include machine and tractor data since they're disabled
      ...
      final Map<String, dynamic> requestData = {
        ...
        // Don't include machine_ids and tractor_ids since they're disabled
        ...
        'hay_making_method': _selectedMethodHayMaking == 'Select Method' ? null : _selectedMethodHayMaking,
      ...
      if (selectedCategories.isNotEmpty) {
        requestData['category'] = selectedCategories.toList();
      } else {
 ...
 _buildCustomTextField(
  ...
  customValidator: (value) {
    ...
    return null;
                  ...
                  _buildCustomDropdown(
                    labelText: 'Method Of Hay Making',
...
final selectedEntry = _plots.firstWhere(
  ...
  // Check if "Select Seed Name" is chosen
  // if (seedName == 'Select Seed Name') {
  //   setState(() {
  //     _selectedSeedName = seedName;
  //     _formFieldsEnabled = false; // Disable form when no seed selected
  //     yieldMt = '';
  //     _resetFormFieldsOnly();
  //   });
  //   return;
  ...
  if (selectedEntry.isNotEmpty) {
    ...
    if (!enableFields) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Hay making is not enabled for $seedName. Form fields are disabled.'),
          backgroundColor: Colors.orange,
          duration: const Duration(seconds: 3),
        ),
      );
